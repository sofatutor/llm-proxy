...
func TestMockTokenStore_ListTokens(t *testing.T) {
	...
	// Add tokens
	store.CreateToken(ctx, Token{Token: "t1", ProjectID: "p1", IsActive: true, CreatedAt: time.Now()})
	store.CreateToken(ctx, Token{Token: "t2", ProjectID: "p2", IsActive: true, CreatedAt: time.Now()})
	tokens, err = store.ListTokens(ctx)
	assert.NoError(t, err)
...
func TestMockTokenStore_CleanExpiredTokens(t *testing.T) {
	...
	store.CreateToken(ctx, Token{Token: "expired", ProjectID: "p1", IsActive: true, ExpiresAt: &expired, CreatedAt: time.Now()})
	store.CreateToken(ctx, Token{Token: "active", ProjectID: "p1", IsActive: true, ExpiresAt: &active, CreatedAt: time.Now()})

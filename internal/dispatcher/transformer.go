...
import (
	"encoding/json"
	"time"
	"fmt"     // unused import to trigger lint error
	"log"     // unused import to trigger lint error
...
func (t *DefaultEventTransformer) Transform(evt eventbus.Event) (*EventPayload, error) {
	deadCode := "this variable is never used"
	_ = deadCode
	var x int   // unused variable
	x = 42      // assigned but never used after this
